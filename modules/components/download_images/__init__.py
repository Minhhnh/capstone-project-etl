...
from modules.components.download_images.resizer import Resizer
...
def is_disallowed(headers, user_agent_token, disallowed_header_directives):
    ...
    for values in headers.get_all("X-Robots-Tag", []):
        try:
            ...
            directives = [x.strip().lower()
                          for x in uatoken_directives[-1].split(",")]
            ua_token = (
                uatoken_directives[0].lower() if len(
                    uatoken_directives) == 2 else None
            )
